...
- **Absolute counter values carry an offset on some units** — and this is a device trait, not a bug. One verified unit reports ~202203 m³ total water after roughly a year of service; the vendor app shows the identical figure. The same applies to the regeneration and maintenance counters. Proof that the offset is a constant pedestal: it cancels out in `total_water − soft_water`, which leaves only the bypass (hard) water. On the verified unit that difference is 41211 L over a documented 601 days of service = 68.6 L/day of bypass; at a 44 % bypass share (blending 18 → 8 °dH) that implies ~154 L/day total, matching the ~180 L/day actually measured by the daily register. The same 601 days against the *absolute* counter would mean 336 m³/day, which is impossible. **Consumption rates are unaffected.** If you want a counter starting from your own commissioning date, put a [`utility_meter`](https://www.home-assistant.io/integrations/utility_meter/) helper on top of the sensor.
