...
- **Operating time** (`/api/rest/2500`) and **installation date** (`/api/rest/0E00`) do **not** reflect when you installed the unit. Two things are established by measurement: `operating_time` in days is *exactly* the calendar-day delta between today and `installation_date` — so only one of the two is a real value, the other is derived — and `installation_date` can be wildly wrong. On a verified i-soft K SAFE+ running SW 4.01, the device reports `2017-05-13` / 3420 days, while the purchase invoice puts the actual installation at `2025-01-30` (601 days) — off by a factor of 5.7. **Why** it reports that particular date is unknown — but it is *not* a firmware build date. That was the long-standing guess here, and it has since been falsified: on the same unit, a firmware update from device-control SW 4.01 → 4.02 (connectivity module 3.51 → 3.52) left `installation_date` at `2017-05-13`, bit for bit. The value survives firmware changes, so it lives somewhere in device storage. If you need time-since-install, track it yourself.
...
- **Water counters are litres, not millilitres.** `/api/rest/2800` (total) and `/api/rest/2900` (soft) return a 4-byte little-endian counter in litres; divide by 1000 for m³. Versions 0.2.2–0.2.4 divided by 1,000,000 and reported everything 1000× too low — fixed in 0.2.5. Verified two ways: the counter's daily increment matches the *separate* daily-consumption register (`FB00`) at a ratio of ~1.0, and raw value `202203299 / 1000 = 202203.3 m³` matches the JU-Control app exactly.
- **Absolute counter values carry an offset on some units** — and this is a device trait, not a bug. One verified unit reports ~202,203 m³ total water after roughly a year of service; the vendor app shows the identical figure. The same applies to the regeneration and maintenance counters. Proof that the offset is a constant pedestal: it cancels out in `total_water − soft_water`, which leaves only the bypass (hard) water. On the verified unit that difference is 41,211 L over a documented 601 days of service = 68.6 L/day of bypass; at a 44 % bypass share (blending 18 → 8 °dH) that implies ~154 L/day total, matching the ~180 L/day actually measured by the daily register. The same 601 days against the *absolute* counter would mean 336 m³/day, which is impossible. **Consumption rates are unaffected.** If you want a counter starting from your own commissioning date, put a [`utility_meter`](https://www.home-assistant.io/integrations/utility_meter/) helper on top of the sensor.
